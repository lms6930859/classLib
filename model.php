...
class Model
{
	...
	public function __call($func , $args)
	{
		...
		if (in_array($func , $arr)) {
			...
			exit($func.'你个王八犊子 你调用的方法没有滚蛋吧娃娃仔');
		}
	}
	...
	public function max($fields)
	{
		...
		$sql="SELECT MAX($fields) AS MAX FROM %TABLE% %WHERE%";
		$sql=str_replace(
		array('%TABLE%','%WHERE%'),
		array(
		$this->parseTable(),
	...
	public function avg($fields)
	{
		...
		$sql="SELECT AVG($fields) AS AVG FROM %TABLE% %WHERE%";
		$sql=str_replace(
		array('%TABLE%','%WHERE%'),
		array(
		$this->parseTable(),
	...
	public function count($fields)
	{
		...
		$sql="SELECT COUNT($fields) AS COUNT FROM %TABLE% %WHERE%";
		$sql=str_replace(
		array('%TABLE%','%WHERE%'),
		array(
		$this->parseTable(),
