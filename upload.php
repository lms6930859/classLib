...
class Upload
{
	...
	public $path = './';
	//准许的mime类型
	public $allowMime = array('image/png','image/jpeg','image/gif','image/wbmp');
	//准许的文件后缀名
	public $allowSub = array('jpeg','png','gif','wbmp','pjpeg','jpg');
	//文件准许的大小
	...
	public function up($fields)  //就是input 表达提交过来的name的值
	{
		...
		if ($this->setFiles($name , $tmpName , $type , $error , $size)) {
			...
			if ($this->checkMime() && $this->checkSize() && $this->checkSub()) {
				...
				if ($this->move()) {
					//echo '';
					$this->newpath='upload/'.$this->newName;
					$this->image=new Image('upload/');
					$newthumbpath=$this->image->thumb($this->newpath,100,100);
				
					// return [$this->path.$this->newName,$this->image];
					return $newthumbpath;
				} else {
					return false;
				}
